...
cd script
2 Generate ESM embeddings: To generate ESM embeddings for the sequence data, run the following command:

python generate_esm.py \
  --seq /path/to/seq/BV_sequences.fasta (BY_sequences.fasta) \
  --datatype BV (or BY) \
  --outdir /path/to/output/directory
Install ESM by following the instructions on the Facebook ESM GitHub repository.
...
3 Generate Input Matrix: After generating ESM embeddings, run the following to generate the input feature matrix:
...
python generate_matrix.py \
  ...
  --dir /path/to/csv \
  --thread 50
4 Train the Model: To train the PREDAC-TransFluB model, run the following:
 ...
 --epoch 200 \
 --number_columns 654
5 Make Predictions: After training, you can use the model to make predictions on new data:
...
6 Cluster Results: Finally, you can perform k-means clustering on the prediction results:
